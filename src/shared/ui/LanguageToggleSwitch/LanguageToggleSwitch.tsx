    ...
    return (
        <div className="flex items-center">
          ...
          >
            ...
            {/* 背景のアクセント（装飾要素） */}
            <span className={`absolute w-24 h-24 rounded-full bg-white opacity-5 transition-all duration-500 ${
          language === 'ja' ? '-top-16 -left-10' : '-top-16 -right-10'
        }`}/>
            
            {/* 言語ラベル - 活性側 */}
            <span className={`absolute left-3 text-xs font-bold z-10 transition-all duration-500 ${
          language === 'ja' 
            ...
            <span className={`absolute right-3 text-xs font-bold z-10 transition-all duration-500 ${
          language === 'en' 
            ? 'text-gray-800 scale-110' 
            : 'text-white/10 scale-90'
        }`}>
            ...
            >
              {/* 丸の中のアクセント */}
              <span className={`absolute inset-0 rounded-full m-1 transition-all duration-500 bg-gray-100`}/>
            </span>
          </button>
        </div>
    );
