...
[[https://github.com/rossmacarthur/sheldon][Sheldon]] は Toml を用いて設定する Zsh / Bash プラグインマネージャーです。

これを Nix 環境で使いたいとき、愚直にやるならば home-manager で plugins.toml を管理すればいいわけですが、home-manager を使って管理できるようになったのでご紹介します。

この変更は home-manager に俺が送った [[https://github.com/nix-community/home-manager/pull/5672][PR]] を引き継いでくれた [[https://github.com/elanora96][elanora96]] さんによって upstream に merge されています。オープンソースソフトウェアにおける暖かさ・自由さを知った一件でした。
...
*** テンプレートエンジン
[[https://sheldon.cli.rs/Examples.html][Sheldon の examples]] には `zsh-defer` を用いたプラグインの遅延ロードについてのテンプレートが書かれています。これは事実上 Sheldon ユーザーはデフォルトで使うものですが、これが Sheldon に組み込まれておらずユーザー側が定義するものという前提で examples に置かれているのは以下のような思想があるからだと邪推します:
...
** 余談
この記事がヒットしそうな検索ワードとして `Nix Sheldon` とかそういうのがありそうですが、世の中には [[https://www.eastern.edu/sheldon-nix][Sheldon Nix]] さんがいるらしいです。すごい名前。

** Reference
この記事で紹介した設定は俺の Nix 設定 ([[https://github.com/Kyure-A/nix-config][Kyure-A/nix-config]]) で実際に使っているものです。

- [[https://github.com/Kyure-A/nix-config/tree/master/inputs/sheldon][inputs/sheldon/]] - Sheldon の子 flake（プラグイン定義・モジュール本体）
- [[https://github.com/Kyure-A/nix-config/blob/master/inputs/sheldon/flake.nix][inputs/sheldon/flake.nix]] - プラグインの flake inputs
- [[https://github.com/Kyure-A/nix-config/blob/master/inputs/sheldon/default.nix][inputs/sheldon/default.nix]] - Sheldon の home-manager モジュール
- [[https://github.com/Kyure-A/nix-config/blob/master/modules/home/programs/zsh/default.nix][modules/home/programs/zsh/default.nix]] - `zsh-defer` と `sheldon source` の読み込み
- [[https://github.com/Kyure-A/nix-config/blob/master/flake.nix][flake.nix]] - ルート flake（子 flake の参照）
