...
この変更は home-manager に私が送った [[https://github.com/nix-community/home-manager/pull/5672][PR]] を引き継いでくれた [[https://github.com/elanora96][elanora96]] さんによって upstream に merge されています。オープンソースソフトウェアにおける暖かさ・自由さを知った一件でした。
...
ここまでの例では Sheldon の `github` ソースと `local` ソースを並列に紹介してきましたが、実際に私が最初に書いていた設定は `github` ソースを使ったものでした。
...
** Reference
この記事で紹介した設定は私の Nix 設定 ([[https://github.com/Kyure-A/nix-config][Kyure-A/nix-config]]) で実際に使っているものです。
